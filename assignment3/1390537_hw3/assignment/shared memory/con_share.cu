...
__constant__ float const_egde[9] = {-1,0,1,-2,0,2,-1,0,1};
__constant__ float const_av[9] = {1/9,1/9,1/9,1/9,1/9,1/9,1/9,1/9,1/9};

__global__ void convolution_shared(float *out,float *matrix,int width,int height,int m_width,int m_height){
    __shared__ float ds_matrix[tile_width][tile_width];
    int j = blockIdx.x*blockDim.x + threadIdx.x;
    int i = blockIdx.y*blockDim.y + threadIdx.y;
    float value = 0;
    ds_matrix[threadIdx.y][threadIdx.x] = matrix[j*width+i];
    __syncthreads();
    for(int p=0;p<m_height;++p){
        for(int l=0;l<m_width;++l){
            ...
            else if((threadIdx.y-(int)(m_width/2)+l)<0){
                value = value + matrix[(j-(int)(m_height/2)+p)*width+(i-(int)(m_width/2)+l)]*const_av[p*m_width+l];
            }
            else if((threadIdx.y-(int)(m_width/2)+l)>=tile_width){
                value = value + matrix[(j-(int)(m_height/2)+p)*width+(i-(int)(m_width/2)+l)]*const_av[p*m_width+l];
            }
            else if((threadIdx.x-(int)(m_height/2)+p)<0){
                value = value + matrix[(j-(int)(m_height/2)+p)*width+(i-(int)(m_width/2)+l)]*const_av[p*m_width+l];
            }
            else if((threadIdx.x-(int)(m_height/2)+p)>=tile_width){
                value = value + matrix[(j-(int)(m_height/2)+p)*width+(i-(int)(m_width/2)+l)]*const_av[p*m_width+l];
            }
            else{
                value = value + ds_matrix[(threadIdx.y-(int)(m_width/2)+l)][(threadIdx.x-(int)(m_height/2)+p)]*const_av[p*m_width+l];
                //value = value + matrix[(j-(int)(m_height/2)+p)*width+(i-(int)(m_width/2)+l)]*const_av[p*m_width+l];
            }
        }
    }
...
void runTest(int argc, char **argv, char *imageFilename)
{
    ...
    convolution_shared<<<dimGrid,dimBlock,0>>>(gData,dData,width,height,3,3);
    ...
    strcpy(global_outputfile + strlen(imagePath) - 4, "_share_out.pgm");
